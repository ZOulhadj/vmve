...
static Device CreateDevice(VkInstance instance,
                                   VkSurfaceKHR surface,
...
static VmaAllocator CreateAllocator(VkInstance instance, uint32_t version, Device& device) {
...
static RendererSubmitContext CreateSubmitContext(Device& device) {
    RendererSubmitContext context{};
    ...
    VkCheck(vkCreateFence(device.device, &fence_info, nullptr, &context.Fence));
    VkCheck(vkCreateCommandPool(device.device, &pool_info, nullptr, &context.CmdPool));
    ...
    VkCheck(vkAllocateCommandBuffers(device.device, &allocate_info, &context.CmdBuffer));

    return context;
}

static void DestroySubmitContext(Device& device, RendererSubmitContext& context) {
...
static VkDescriptorPool CreateDescriptorPool(Device& device, const std::vector<VkDescriptorPoolSize>& sizes, uint32_t max_sets) {
    ...
    VkCheck(vkCreateDescriptorPool(device.device, &pool_info, nullptr, &pool));
...
RendererContext* CreateRendererContext(uint32_t version,
                                       ...
                                       const Window* window) {
    auto context = (RendererContext*)malloc(sizeof(RendererContext));
    ...
    context->allocator = CreateAllocator(context->instance, version, context->device);
    context->submit    = CreateSubmitContext(context->device);
    context->compiler  = CreateShaderCompiler();
    ...
    context->pool = CreateDescriptorPool(context->device, pool_sizes, 1000 * pool_sizes.size());
...
void DestroyRendererContext(RendererContext* rc) {
    ...
    DestroyShaderCompiler(rc->compiler);
    DestroySubmitContext(rc->device, rc->submit);
