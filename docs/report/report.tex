...
// TODO: Project should be presented as a learning tool that bridges the gap for new users
...
The end-users are the main stakeholders of the final-year project. 


The main stakeholder for this project is the end users. Users are the
individuals that will test and use the application once it has been developed.

The main aims and objectives of this project will focus on these users to ensure
that all their needs are fulfilled.

\subsubsection{Students}
\subsubsection{Trainees}
\subsubsection{Employers}
...
\gls*{vmve} has several aims which describe the general concepts that the
application wishes to provide.

\begin{description}
  \item[1. A platform for virtual environment rendering] The first aim of
  ...
  \item[2. Lower hardware requirements] The second aim of the application is to be
  ...
  \item[3. Provide secure 3D asset encryption] The last of the three main goals is
...
\begin{description}
  ...
  \item[2. Lower hardware requirements]
  \item[3. Provide secure 3D asset encryption]
\end{description}
...
This project combines the knowledge gained throughout the degree and presents a
highly technical application that uses advanced concepts.
...
\begin{description}
  \item[Design (\ref{design})] Focuses on the concept of the project, the design
    ...
    followed by the design of the application including the user interface and
  ...
  \item[Evaluation (\ref{evaluation})] comes after the implementation and the
    main goal of this section is to ensure that the application has met the aims
    and objectives outlined in the projects.
  \item[Future Work (\ref{future_work})] is the final key section that aims to
    outline the aims and specific features that will need to be implemented
    going forward in regards to the project.
\end{description}
...
Throughout this comparison, the choice of tools is heavily influenced by the
aims and objectives outlined in sections \ref{aims} and \ref{objectives} 
respectively.
...
\subsubsection{Development Enviornment}
- VS 2022 
- VSCode 
- Clion
- 
...
The application will be desktop based...
...
\subsubsection{Rendering API} \label{rendering_api_tech_review}
...
\subsubsection{Externel Libraries}
...
\subsection{Tools}
\subsubsection{Version control} \label{project_management}
...
\subsubsection{Choice of Rendering API}
